...
async function getSubjectTimeline(
  args: z.infer<typeof GetSubjectTimelineSchema>
): Promise<unknown> {
  const { firebase_uid, subject_label, limit, signal_type, before_iso } = args;

  const params: unknown[] = [firebase_uid, subject_label, limit];
...
async function analyzeBiometricCorrelations(
  args: z.infer<typeof AnalyzeBiometricCorrelationsSchema>
): Promise<unknown> {
  ...
  if (hiProt && loProt && hiProt - loProt >= 0.5) {
    observations.push(
      `Meals with ≥30g protein show a satiety score ${(hiProt - loProt).toFixed(1)} points higher on average.`
    );
  }
  ...
  if (bloatTotal > 0 && (bloatSevere + bloatMod) / bloatTotal > 0.2) {
    observations.push(
      `${Math.round(((bloatSevere + bloatMod) / bloatTotal) * 100)}% of meals in this window showed moderate or severe bloating.`
    );
  }
...
async function registerNewSubject(
  args: z.infer<typeof RegisterNewSubjectSchema>
): Promise<unknown> {
...
const server = new Server(
  { name: "biofeedback-openbrain", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
  ...
  tools: [
    {
      ...
      description:
        "Retrieve an ordered history of biometric signals for a subject. Returns raw payload and AI enrichment fields. Supports pagination via before_iso cursor and filtering by signal type.",
      inputSchema: {
    ...
    },
    {
      ...
      description:
        "Aggregate statistics and pattern observations for a subject's meal signals over a configurable look-back window. Returns protein/satiety cohort comparison, bloating frequency, meal-type breakdown, and plain-language observations.",
      inputSchema: {
        type: "object" as const,
        properties: {
    ...
    },
    {
      ...
      description:
        "Idempotently create an account row (keyed by Firebase UID) and a subject row. Safe to call multiple times — uses ON CONFLICT upsert. Returns the account and subject IDs.",
      inputSchema: {
        type: "object" as const,
        properties: {
          firebase_uid: { type: "string", description: "Firebase Auth UID" },
