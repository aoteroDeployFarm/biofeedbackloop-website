...
function getSafetySettings() {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const { HarmCategory, HarmBlockThreshold } = require("@google/genai") as typeof import("@google/genai");
  return [
...
if (process.env.K_SERVICE) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const { onCall, onRequest, HttpsError } = require("firebase-functions/v2/https") as {
    onCall: typeof import("firebase-functions/v2/https").onCall;
    onRequest: typeof import("firebase-functions/v2/https").onRequest;
    HttpsError: typeof import("firebase-functions/v2/https").HttpsError;
  };
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const { onDocumentCreated } = require("firebase-functions/v2/firestore") as {
    onDocumentCreated: typeof import("firebase-functions/v2/firestore").onDocumentCreated;
  };
  // eslint-disable-next-line @typescript-eslint/no-var-requires
...
JSON:`;
  ...
  exports.generateInsight = onRequest(
    ...
    async (req, res) => {
      ...
      if (!validPath) { res.status(404).json({ error: "Not found" }); return; }
...
Return only the observation text — no labels, no JSON, no markdown.`;
  ...
  exports.transcribeMeal = onRequest(
    ...
    async (req, res) => {
      ...
      if (!audio?.trim()) { res.status(400).json({ error: "No audio data received" }); return; }
...
Set "isDone" to true ONLY if the user utters an explicit sign-off keyword at the very end of their speech: "done", "log it", or "sync". If none of these exact words appear, set "isDone" to false.`,
        ...
        let parsed: { rawTranscript?: string; isDone?: boolean };
        try {
        ...
        if (!text) { res.status(404).json({ error: "No speech detected in the recording" }); return; }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const { uid, signalId } = event.params as { uid: string; signalId: string };
    ...
    try {
      ...
      const firestoreType = (data["type"] as string) ?? "meal";
      ...
      const signalType: string = SIGNAL_TYPE_MAP[firestoreType] ?? "meal";

      // Live column is "timestamp", not "recorded_at"
      const signalTimestamp: Date =
        (data["timestamp"] as { toDate?: () => Date })?.toDate?.() ?? new Date();
      const rawPayload: Record<string, unknown> =
        (data["payload"] as Record<string, unknown>) ?? {};
      const aiEnrichment: Record<string, unknown> = {
      ...
      const subjectRes = await pool.query(
        `INSERT INTO subjects (account_id, label, name, type)
         VALUES ($1, 'self', 'Self', 'human')
         ON CONFLICT (account_id, label) DO NOTHING
         RETURNING id`,
        [accountId]
      );
      let subjectId: string = subjectRes.rows[0]?.id;
      if (!subjectId) {
        // DO NOTHING fired (row already existed) — fetch the existing id
        const existing = await pool.query(
          `SELECT id FROM subjects WHERE account_id = $1 AND label = 'self'`,
          [accountId]
        );
  ...
  function stub(endpoint: Record<string, unknown>): (...args: unknown[]) => Promise<void> {
    const fn = async (..._args: unknown[]): Promise<void> => { /* never called during spec-discovery */ };
    (fn as unknown as Record<string, unknown>).__endpoint = endpoint;
    return fn;
  }
